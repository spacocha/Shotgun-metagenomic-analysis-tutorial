...
TPM_genes_2=TPM_final_table_KO_neg_merged_divided_final{:,indenv+1};
...
%Make frac of total values
nlnosZbacter=nosZ_taxa(1,:);
nlnosZgamma=nosZ_taxa(2,:);
nlnosZall=metabolicgenesnoatp(konoatp==376,:);

nnfracgamma=nlnosZgamma./nlnosZall;
nnfracbacter=nlnosZbacter./nlnosZall;

fracgammanonApril=mean(nnfracgamma(Cruise==2|Cruise==3|Cruise==4));
fracgammaApril=mean(nnfracgamma(Cruise==1)/nlnosZall(Cruise==1));
fracbacternonApril=mean(nnfracbacter(Cruise==2|Cruise==3|Cruise==4));
fracbacterApril=mean(nnfracbacter(Cruise==1));
